...
async function act2_CharacterSelection(scene) {
  ...
  // Store selected character and others for scene_02
  window.playerCharacter = character;
  window.aiCharacters = aiCharacters;
  
  console.log('Character selection complete. Transitioning to battle...');
  
  // Transition to scene_02 (battle scene)
  const { run: runScene02 } = await import('./scene_02.js');
  await runScene02({ scene });
}
